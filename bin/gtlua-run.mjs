// gtlua-run.mjs - play a .gtr in a window, zero external emulator install.
//
// Loads the bundled GameTank libretro core (romdev-core-gametank) and drives a
// minimal frame loop: retro_run each tick, blit the framebuffer to a @kmamal/sdl
// window, and map the keyboard to the GameTank pad. This is OUR small host - it
...
//
// GameTank pad (libretro RetroPad bit -> GameTank button):
//   d-pad = UP/DOWN/LEFT/RIGHT, A(O)=RETRO_A, B(X)=RETRO_B, C=RETRO_Y,
//   START=RETRO_START. See the SDK's btn() index doc (0-3 d-pad, 4=A, 5=B,
//   6=C, 7=START).
//
...
function keymap(sdl) {
  ...
  return [
    ...
    [K.Z, ID.A], [K.X, ID.B], [K.C, ID.Y], [K.RETURN, ID.START], [K.RSHIFT, ID.SELECT],
  ];
}
...
export async function runRom(romPath, opts = {}) {
  ...
  const { core } = await import("romdev-core-gametank");
  if (!existsSync(core.jsPath)) throw new Error("romdev-core-gametank wasm missing (run: npm install)");
  ...
  let fbWidth = 128, fbHeight = 128, pixelFormat = RETRO_PIXEL_FORMAT_XRGB8888;
  ...
  const window = sdl.video.createWindow({
    ...
    width: 128 * scale, height: 128 * scale, resizable: true,
  });
  let audioDev = null;
  try {
