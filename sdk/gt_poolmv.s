...
; ---------------------------------------------------------------------------
; gt.pool_decay - per-slot table-lookup accumulate + saturating decrement
; in one walk: for every slot with act[i] != 0:
;   sum += table[act[i] - 1];  lm[i] = max(0, lm[i] - step)
; act is an INT array (stride 2, low byte = a 1..N table index), lm and
; table are BYTE arrays, step is the per-frame decrement (pd_step).
; Returns the sum (int in A/X). ~30 cycles a slot against ~350 compiled.
; Reuses pm_x (act), pm_sx (lm), pm_sy (table), pm_n.
; ---------------------------------------------------------------------------
.export _gt_pool_decay_z
.export _pd_step
